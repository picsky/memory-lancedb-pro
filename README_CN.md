...
| `src/memory-consolidation.ts` | 6 维巩固评分引擎：访问频次、确认使用、矛盾惩罚等 |
| `src/background-scheduler.ts` | 三级后台处理管道：Sweep → Consolidate → Compact |
| `src/access-tracker.ts` | 动态置信度更新：确认有用加分、矛盾减分、长期不使用衰减 |
...
当 LLM 一次性打分的 `importance` 与实际使用情况不匹配时，6 维巩固评分模型能更准确地判断记忆价值：

| 维度 | 权重 | 说明 |
|---|---|---|
| access_recency | 0.25 | 最近是否被召回（14天半衰期） |
| access_frequency | 0.20 | 累计召回次数（对数饱和） |
| injection_use | 0.20 | 注入上下文的次数 |
| confirmation | 0.15 | 被确认有用的信号 |
| bad_recall | 0.15 | 被否定/无用的惩罚 |
| tier_stability | 0.05 | 在当前 tier 的稳定性 |

**输出动作**：
- `score >= 0.75` → 提升到 core tier
- `score <= 0.15` + 年龄 >= 30天 → 标记 archive
- `bad_recall_count >= 5` → 自动 suppress

### 动态置信度更新（新增）

每次召回结果都会影响记忆的 `confidence` 值：
- 被确认有用 → confidence += delta
- 被 contradicted → confidence -= delta
- 长期不使用 → 缓慢衰减（30天后开始，每天 0.001）
- 最小值 0.1，最大值 1.0
...
| 阶段 | 触发时机 | 操作 |
|---|---|---|
| **Sweep** | 每次 gateway_start | 计算 decay score、标记 stale、计算 health |
| **Consolidate** | cooldown 24h | 6 维巩固评分 + tier 迁移 |
| **Compact** | cooldown 24h（现有） | LSH 聚类 + 相似合并 |

### 记忆健康与自动恢复（新增）

- health = core_active_ratio * 0.4 + avg_confidence * 0.3 + recall_success_rate * 0.3
- health < threshold 时，自动从 archived 中恢复高 confidence (>=0.6) 的条目
...
| **6 维巩固评分** | 结合访问模式、确认使用、矛盾惩罚等多信号，弥补 LLM 一次性 importance 评分的不足。 |
| **动态置信度** | 每次召回结果实时调整 confidence 值，被确认有用加分、被否定减分、长期不使用缓慢衰减。 |
| **三级后台管道** | Sweep（每次启动）→ Consolidate（24h）→ Compact（24h），全自动后台处理。 |
| **记忆健康监控** | 复合健康指标，低于阈值时自动恢复高价值 archived 记忆。 |

**基准测试结果**：当 importance（LLM 一次性评分）与实际使用不相关时，巩固评分实现 100% precision@K（baseline 为 0%），消除 46pp 噪音，tier 分类准确率 98%+。
